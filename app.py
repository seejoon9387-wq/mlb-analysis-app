...
team_map = {
    ...
    "Miami Marlins": "마이애미 말린스", "St. Louis Cardinals": "세인트루이스 카디널스",
...
def get_game_data():
    ...
    try:
        ...
        game_list = []
        for g in games:
            # 시간 정보: API의 game_time 대신, 상세 정보에서 추출 시도
            # 시간 데이터가 없으면 '경기 시간 미정'
            time_str = g.get('game_time', '미정')
            
            game_list.append({
                "날짜": g.get('game_date'),
                "시간": time_str,
                "원정팀": team_map.get(g.get('away_name'), g.get('away_name')),
                "원정 선발": g.get('away_probable_pitcher', '미정'),
                "홈팀": team_map.get(g.get('home_name'), g.get('home_name')),
